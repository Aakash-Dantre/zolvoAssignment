...
constructor(props){
   ...
   onSubmit(e){
    ...
    axios.post('http://localhost:8000/credit/',data)
         .then(res => {
            this.setState({balance:res.data.balance})
            localStorage.setItem('mydata',JSON.stringify(this.state))
            console.log(localStorage['mydata'])
            document.getElementById("AfterSubmit").innerHTML = "Credit Successfull" ;


         })
         .catch(function(error) {
             console.log(error);
            document.getElementById("AfterSubmit").innerHTML = "Credit FAILED" ;
    ...
    return(
      <div>
         ...
         <form onSubmit={this.onSubmit}>
                    ...
                    <div className="form-group">
                        <input type="submit" value="Credit" className="btn btn-primary"/>
                    </div>
